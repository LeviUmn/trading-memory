...
- [N1 Tweet-Fälligkeit + F-B **FREIGEGEBEN MIT AUFLAGEN** (Opus-Gegencheck 14.09.) — P2-Blocker behoben](project_n1_tweet_faelligkeit_klaerung_2026-09-14.md) — `x_last_fetch.json`: `last_fetch` (Tweet-Wasserstand) + `last_poll` (letzter Abruf) + `prev_poll`; (b)/Nachholen gegen `last_poll`, slot-basiert (Rundungsfalle gelöst); neu `--polled`, Pflicht nach JEDEM Abruf und im Cron-Prompt verankert. Opus-Lauf 37 VC/3 h: **0 falsche Nachholen-JA** (alte Semantik 37/37) — **ohne den Stempel kehrt der Blocker aber zu 12/13 zurück (N-1: nicht maschinell erzwungen)**. **A-1 vor dem Testtag: Live-Datei hat noch kein `last_poll` → einmal stempeln, sonst läuft der Migrations-Fallback = altes Verhalten.** N-2 (−30-s-VC-Fenster), N-3 (keine Mindestpause), N-4 (`--polled` rückwärts). O-1: Stempelpflicht in [[feedback_loop_ablauf_uebersicht]] Schritt 6. Tests 90/90
