...
- [Opus+Fable-Vollcheck 24.08.2026: N-1 bis N-22 abgearbeitet, D-1 entschieden+umgesetzt, D-3/D-4/D-5 offen](project_opus_vollpruefung_2026-08-24.md) — C-1 bis C-4 + gate_check.cjs umgesetzt, Phase-3 korrigiert (RR 1,03:1, EV −0,059%). Alle 17 direkt umsetzbaren N-Punkte erledigt (Q-Score UNBEKANNT-Zustand statt falsch-ROT, Cluster-Gate an ATR gekoppelt, trades.sql/trades_dax.sql neu gezogen+committet, trade_stats.cjs um TP1-/TP2-Quote+exit_type erweitert, --violation-0-Bug, runway_ratio-Formel vereinheitlicht, retroaktiver Gate-Check #36-43 in DB zurückgeschrieben — 6/8 FAIL, u.a.). **Nachtrag spät: D-1 von Levi entschieden** — RR-Floor für die 8b1-Schritt-5-TP1-Kandidatenwahl pauschal zurück auf 1:1 für BEIDE Zonen (nicht Fables zonenabhängige Option A1), Drei-Zonen-Distanzfilter selbst unverändert scharf; `gate_check.cjs` `TP1_SELECTION_RR_THRESHOLD` auf `{1:1.0, 2:1.0}` gesetzt, `feedback_chartanalyse.md` 8b1 Schritt 5/6 nachgezogen. **Zusätzlich neu: 8b2-TP2-Realismus-Gate** (Levi-Auftrag im selben Zug) — TP2-Kandidat muss echtes Chart-Level (Pine-Lines/-Labels/-Boxes) UND RR≥2:1 sein; fehlt ein solches Level, KEIN Ausschluss, nur halbe Position (Stacking-sicher); Parameter fehlt → UNKLAR statt FAIL/PASS (GC-1-konform). Neues Gate `tp2RealismGate` in `gate_check.cjs`, Live-Parameter `--tp2-level-price`, drei Testfälle live verifiziert (volle Position / halbe Position / UNKLAR-ohne-FAIL), Commit erfolgt (kein Push). D-3 (#41-43-Phasenzuordnung), D-4 (#40-Review/Phase-4-Gates-Zwischenstand) und D-5 (Testfenster-Enderkriterium vs. Früh-Exit-Stack) weiterhin offen, unangetastet
