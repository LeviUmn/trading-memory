...
1. **TP2-Kandidat muss ein echtes Chart-Level sein** — dieselbe Quelle wie die bestehende TP1-Realismus-Prüfung: Pivot (7a1a), Vortageshoch/-tief (PDH/PDL), VWAP-Band, Box-Kante o.ä., ermittelt über `data_get_pine_lines`/`data_get_pine_labels`/`data_get_pine_boxes` — **nicht** nur eine ATR-Rechnung (der Unterschied zur Zonen-Distanzprüfung in 8b1, die bewusst ATR-basiert bleibt, weil sie eine reine Plausibilitäts-Außengrenze ist, keine Levelsuche).
