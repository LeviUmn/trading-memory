...
metadata: 
  ...
  modified: 2026-09-16T09:57:48.154Z
---
...
**Skript-Logs sind die PRIMÄRQUELLE der Rekonstruktion — das Chat-Transkript nur Ergänzung für Freitext (V7, ergänzt 16.09.2026, Opus-Analyse Testtag 15.09.2026, Levi-Freigabe):** Fehlt die während des Tages mitgeschriebene Protokolldatei (wie am 15.09.2026), wird das Faktenprotokoll AUSSCHLIESSLICH primär aus den Skript-Logs rekonstruiert: `gate_check_log.jsonl` (jeder Gate-Aufruf mit Ein-/Ausgaben), `vollcheck_state.json` + `vollcheck_log.jsonl` (V4, seit 16.09.2026: jeder vollcheck.cjs-Lauf mit Nr./ts/Lücken/Hard-Exit-Grund, unabhängig vom Chat-Redirect), `oneh_shadow_log.jsonl`, `trigger_kandidaten_log.jsonl`, `register_touch_log.jsonl`, `sl_anker_wechsel_log.jsonl`, `skipped_setups_fiktiv.jsonl`, `loop_stopp_log.jsonl` sowie die `screenshots/`-Dateiliste (mtime). Das Chat-Transkript dient NUR noch als Ergänzung für Freitext-Inhalte, die kein Log trägt (Begründungssätze, Levi-Anweisungen, Quick-Tick-Wortlaut) — NIE als Quelle für Zahlen, die ein Log exakt führt. **Beleg (15.09.2026):** Die Transkript-Rekonstruktion setzte 3 Zahlen falsch (u.a. "2 PASS/2 UNKNOWN" statt real 0/0, "~5 Vorprüfungen" statt real 93) und gab 4 Zählstände fälschlich als "nicht auszählbar" auf (Screenshots, Register-Touches, Skipped-Setups) — ALLE waren aus den Skript-Logs exakt rekonstruierbar. Widerspricht das Transkript einem Skript-Log, gilt das Log; die Abweichung wird in Unterpunkt 4 (Brüche) ausgewiesen.

**V3-Auto-Erfassung und die Nachtragspflicht (ergänzt 16.09.2026, Opus-Gegencheck V1-V7, Nachbesserung 5):** Seit V3 schreibt `gate_check.cjs` an fiktiven Testtagen jeden Live-FAIL AUTOMATISCH nach `skipped_setups_fiktiv.jsonl` — dadurch entstehen deutlich mehr Zeilen als bisher (am 15.09. wären es statt 1 Eintrag ~2 Setups mit Wiederholungszählern gewesen). Diese automatisch erfassten Zeilen unterliegen derselben Nachtragspflicht wie manuelle (Entscheidung + hypothetisches Ergebnis per `skipped_fiktiv.cjs --nachtrag <id>` beim Tagesabschluss), und `protokoll_bilanz.cjs` wird deshalb künftig ÖFTER mit Exit 1 (offene Nachträge) enden — das ist gewollt und kein Fehler des Skripts: Exit 1 heißt "Nachträge offen", die Liste abarbeiten statt den Exit-Code wegzuerklären.
